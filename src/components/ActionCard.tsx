...
function ActionCard({ action, onClick }: { action: QuickActionType; onClick: () => void }) {
  return (
    <Card
      className="group relative overflow-hidden hover:border-blue-700 transition-all duration-300 hover:shadow-lg cursor-pointer"
      onClick={onClick}
    >
      {/* ACTION GRADIENT */}
      <div
      ...
      <div className="relative p-6 size-full">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <h3 className="font-semibold text-xl group-hover:text-blue-700 transition-colors">
              {action.title}
            </h3>
            <p className="text-sm text-muted-foreground group-hover:text-blue-700 transition-colors">
              {action.description}
            </p>
          </div>
        </div>
